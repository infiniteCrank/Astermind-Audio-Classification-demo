// src/worker.ts
// Standardizes features (z-score) so train/test match; supports KernelELM + ELM.
...
type InitMsg = { action: 'init' };
type InMsg = (TrainMsg | PredictMsg | InitMsg) & { id: string };
...
function fitScaler(X: number[][]): { mu: number[], sigma: number[] } {
  ...
  for (let j = 0; j < d; j++) varr[j] = Math.sqrt(varr[j] / Math.max(1, n - 1)) || 1e-6;
  return { mu, sigma: varr };
}
function transformOne(x: number[], sc: { mu: number[], sigma: number[] }): number[] {
  const d = x.length; const y = new Array(d);
  for (let j = 0; j < d; j++) y[j] = (x[j] - sc.mu[j]) / (sc.sigma[j] || 1e-6);
  return y;
}
  ...
  try {
    ...
    if (action === 'train') {
      ...
      if (typeof KernelELM !== 'undefined' && KernelELM) {
        ...
        const defaults = {
          categories: Array.from({ length: outDim }, (_, i) => String(i)),
          outputDim: outDim,
          hiddenUnits: 128,
